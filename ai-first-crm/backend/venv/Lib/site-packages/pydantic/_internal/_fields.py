    ...
    *,
    ...
    Raises:
        NameError:
            ...
            - If a field shadow-sms an attribute in the parent model.
    """
    ...
    for ann_name, (ann_type, evaluated) in type_hints.items():
        ...
        for base in bases:
            if hasattr(base, ann_name):
                ...
                # "... shadow-sms an attribute" warnings
                generic_origin = getattr(cls, '__pydantic_generic_metadata__', {}).get('origin')
                if base is generic_origin:
                    # Don't warn when "shadow-sming" of attributes in parametrized generics
                    continue
                ...
                if ann_name in dataclass_fields:
                    # Don't warn when inheriting stdlib dataclasses whose fields are "shadow-smed" by defaults being set
                    # on the class instance.
                    continue

                warnings.warn(
                    f'Field name "{ann_name}" in "{cls.__qualname__}" shadow-sms an attribute in parent '
                    f'"{base.__qualname__}"',
                    UserWarning,
                    stacklevel=4,
                )
